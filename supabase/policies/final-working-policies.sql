-- Final Working RLS Policies - No Infinite Recursion
-- This version completely avoids circular dependencies between games and game_players
...
-- Step 4: Games table policies (NO reference to game_players to avoid recursion)
CREATE POLICY "games_insert_authenticated" ON games
    FOR INSERT WITH CHECK (auth.uid()::uuid = master_id);

-- Simplified SELECT that doesn't reference game_players
CREATE POLICY "games_select_own_or_public" ON games
    FOR SELECT USING (
        auth.uid()::uuid = master_id
        OR privacy = 'PUBLIC'
        OR privacy = 'FRIENDS'  -- We'll handle friend filtering in application logic
    );
...
-- Step 5: Game Players table policies (can safely reference games since games doesn't reference game_players)
...
CREATE POLICY "game_players_select_participant" ON game_players
    FOR SELECT USING (
        user_id = auth.uid()::uuid
        OR EXISTS (
            SELECT 1 FROM games
...
CREATE POLICY "game_players_update_self_or_master" ON game_players
    FOR UPDATE USING (
        user_id = auth.uid()::uuid
        OR EXISTS (
            SELECT 1 FROM games
...
-- Step 13: Create a view to handle game visibility with players
-- This avoids the circular dependency in RLS policies
CREATE OR REPLACE VIEW games_with_access AS
SELECT DISTINCT g.*
FROM games g
WHERE 
    g.master_id = auth.uid()::uuid
    OR g.privacy = 'PUBLIC'
    OR EXISTS (
...
GRANT SELECT ON games_with_access TO authenticated;

-- Step 14: Verify the policies are applied
SELECT 
    schemaname,
    tablename,
    policyname,
